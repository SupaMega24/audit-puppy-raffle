...
contract PuppyRaffle is ERC721, Ownable {
    ...
    function enterRaffle(address[] memory newPlayers) public payable {
        // my qs: were custom reverts avaible for this version of solidity?
        // my qs: what if no newPlayers and msg.value is 0?
        require(
            msg.value == entranceFee * newPlayers.length,
            "PuppyRaffle: Must send enough to enter raffle"
        );
        for (uint256 i = 0; i < newPlayers.length; i++) {
            // my qs: What resets the players array?
            players.push(newPlayers[i]);
        }

        // Check for duplicates
        // potential bug: denial of service attack if too many players
        // @audit DoS
    ...
    function getActivePlayerIndex(
        ...
        // my qs: What if player is at index 0?
        // @audit if player is at 0, it will return 0 and player might think they are not active!
        return 0;
    }
